...
##
Pre_EC_Path = os.path.join(preproc_directory, 'Pre', 'EyesClosed')

Pre_EC_Plots = os.path.join(fooof_directory, 'Plots', 'Pre', 'EyesClosed')
os.makedirs(Pre_EC_Plots, exist_ok=True)

Pre_EC_summary = os.path.join(fooof_directory, 'Model_Summary', 'Pre', 
                                 'EyesClosed')
os.makedirs(Pre_EC_summary, exist_ok=True)

Pre_EC_periodic = os.path.join(fooof_directory, 'Periodic', 'Pre', 
                                  'EyesClosed')
os.makedirs(Pre_EC_periodic, exist_ok=True)

Pre_EC_aperiodic = os.path.join(fooof_directory, 'Aperiodic', 'Pre', 
                                   'EyesClosed')
os.makedirs(Pre_EC_aperiodic, exist_ok=True)

Pre_EC_aperiodic_param = os.path.join(fooof_directory, 'Aperiodic', 'Parameter', 'Pre', 
                                   'EyesClosed')
os.makedirs(Pre_EC_aperiodic_param, exist_ok=True)
...
##
Post_EC_Path = os.path.join(preproc_directory, 'Post', 'EyesClosed')

Post_EC_Plots = os.path.join(fooof_directory, 'Plots', 'Post', 'EyesClosed')
os.makedirs(Post_EC_Plots, exist_ok=True)

Post_EC_summary = os.path.join(fooof_directory, 'Model_Summary', 'Post', 
                                 'EyesClosed')
os.makedirs(Post_EC_summary, exist_ok=True)

Post_EC_periodic = os.path.join(fooof_directory, 'Periodic', 'Post', 
                                  'EyesClosed')
os.makedirs(Post_EC_periodic, exist_ok=True)

Post_EC_aperiodic = os.path.join(fooof_directory, 'Aperiodic', 'Post', 
                                   'EyesClosed')
os.makedirs(Post_EC_aperiodic, exist_ok=True)

Post_EC_aperiodic_param = os.path.join(fooof_directory, 'Aperiodic', 'Parameter', 'Post', 
                                   'EyesClosed')
os.makedirs(Post_EC_aperiodic_param, exist_ok=True)


input_paths = [Pre_EO_Path, Pre_EC_Path, Post_EO_Path, Post_EC_Path]
plot_paths = [Pre_EO_Plots, Pre_EC_Plots, Post_EO_Plots, Post_EC_Plots]
summary_paths = [Pre_EO_summary, Pre_EC_summary, Post_EO_summary, 
                 Post_EC_summary]
periodic_path = [Pre_EO_periodic, Pre_EC_periodic, Post_EO_periodic,
                 Post_EC_periodic]
aperiodic_path = [Pre_EO_aperiodic, Pre_EC_aperiodic, Post_EO_aperiodic,
                 Post_EC_aperiodic]
parameter_path = [Pre_EO_aperiodic_param, Pre_EC_aperiodic_param, Post_EO_aperiodic_param,
                  Post_EC_aperiodic_param]
conditionnames = ['pre_Average_EO', 'pre_Average_EC', 'post_Average_EO', 'post_Average_EC']

def fooof_func(condition):
    # List of files and participant IDs
    files = os.listdir(input_paths[condition])
    Participants = [file.split('_')[0] for file in 
	...
	# Test if file exist
        file_path1 = os.path.join(periodic_path[condition], f'{Participant}_{conditionnames[condition]}_periodic.csv')
        file_path2 = os.path.join(aperiodic_path[condition], f'{Participant}_{conditionnames[condition]}_aperiodic.csv') 
        file_path3 = os.path.join(parameter_path[condition], f'{Participant}_{conditionnames[condition]}_aperiodic_parameter.csv')
           
        if (os.path.isfile(file_path1) and os.path.isfile(file_path2) and os.path.isfile(file_path3)):
            continue

        print(f'Condition {condition+1}/4.Running Participant {run+1}/{len(Participants)}')
        ...
        aperiodic_params = pd.DataFrame(np.zeros((59,3)))             
