function [RESULTS] =  prepare_my_vectors_erp_parfor(training_set, test_set, cfg)
%
...
RESULTS = struct();
prediction_accuracy = zeros(floor((size(training_set{1,1,1,1}, 1) - (cfg.window_width - cfg.step_width)) / cfg.step_width), size(test_set, 3), crossval_repetitions(1, 1));
feature_weights = cell(floor((size(training_set{1,1,1,1}, 1) - (cfg.window_width - cfg.step_width)) / cfg.step_width), size(test_set, 3), crossval_repetitions(1, 1));
feature_weights_corrected = cell(floor((size(training_set{1,1,1,1}, 1) - (cfg.window_width - cfg.step_width)) / cfg.step_width), size(test_set, 3), crossval_repetitions(1, 1));
perm_prediction_accuracy = zeros(floor((size(training_set{1,1,1,1}, 1) - (cfg.window_width - cfg.step_width)) / cfg.step_width), size(test_set, 3), repetition_data_steps(crossval_repetitions(2, 1)));

crossval_steps = size(test_set, 3);
conditions = size(test_set, 2);
...
for main_analysis = 1:nr_rounds % 1=real decoding, 2=permutation test
    ...
    parfor rep = 1:crossval_repetitions(main_analysis, 1) % repetition of cross_validation cycle   MAKE PARFOR AGAIN
        ...
        for cv = 1:crossval_steps % cross-validation step
            ...
            for s = 1:nsteps
                ...
                for con = 1:conditions % condition 
                    ...
                    if cfg.stmode == 1
                        ...
                        if main_analysis == 2
                            ...
                            new_labels_train = train_new_labels(labels_train, perm_order);
                          labels_train = [];
                ...
                if main_analysis == 1
                    ...
                    prediction_accuracy(s, cv, rep) = acc;
                    feature_weights{s, cv, rep} = feat_weights;
                    feature_weights_corrected{s , cv, rep} = feat_weights_corrected;                        
                                            
                elseif main_analysis == 2
    
                    perm_prediction_accuracy(s, cv, rep) = acc;
                    
                end % of if main_analysis    
                                    
              acc = [];
              feat_weights = [];
              feat_weights_corrected = [];
    ...
    RESULTS = struct('prediction_accuracy', {{prediction_accuracy}}, ...
    'feature_weights', {{feature_weights}}, ...
    'feature_weights_corrected', {{feature_weights_corrected}}, ...
    'perm_prediction_accuracy', {{perm_prediction_accuracy}}); 
