...
addpath(genpath(filename));  % Add path including subfolders.

%% Add EEGLAB to search path
% Important - add path to where EEGlab is housed on your computer.
addpath(genpath(fullfile(filename, '..\eeglab2023.0')));
addpath(genpath(fullfile(filename, '..\Data\')));
...
study_name = 'MVPA_Total_Agreeableness_Pre_EO';
...
if ~(isfolder(fullfile(filename, '../Output')))                
    mkdir(fullfile(filename, '../Output'))
end
output_dir = [fullfile(filename, '../Output')];
    
% Filepaths of single subject datasets (relative to the base directory)
sbj_code = {['Data/Ready_for_DDTBOX/EEG_sorted_cond/Total/Pre eyes open/eeg_sorted_cond']}; % participants EEG
sbj_code_bfi = {['Data/Ready_for_DDTBOX/Personality_Scores/Agreeableness/eeg_sorted_cond_regress_sorted_cond']}; % participants BFI
...
%channel_names_file = 'eeglab_chanlocsfile.mat'; % Name of the .mat file containing channel labels and channel locations
%channellocs = [bdir]; % Path of the directory containing channel information file
...
dcg_labels{1} = 'Level_of_Agreeableness';
%dcg_labels{2} = 'B vs. D';
...
for dcg_set = 1:length(dcgs_for_analyses)
    ...
    for sbj = sbj_todo
        ...
        cfg.data_open_name = [fullfile(fileparts(bdir), [(sbj_code{sbj}) '.mat'])];
        cfg.data_save_name = [fullfile(fileparts(bdir), [(sbj_code{sbj}) '_data.mat'])];
        cfg.regress_label_name = [fullfile(fileparts(bdir), [sbj_code_bfi{sbj} '.mat'])]; % Filepath for regression labels file
