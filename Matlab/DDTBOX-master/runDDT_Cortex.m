function runDDT_Cortex(i_trait, i_cond, cross_val_steps, n_rep_cross_val, permut_rep) 
...
fprintf("The Input for runDDT is: i_trait %s,\n i_cond %s,\n cross_val_steps %s,\n n_rep_cross_val %s,\n permut_rep %s\n", ...
    num2str(i_trait), num2str(i_cond), num2str(cross_val_steps), num2str(n_rep_cross_val), num2str(permut_rep))
...
traits = ["Conscientiousness", "Extraversion", "Neuroticism", "Openness", "Agreeableness"];
Eye_conditions = ["EyesOpen", "EyesClosed"];
...
% Adjust Folders Here        
Datafolder = fullfile('Cortex-Data', filesep);    
%addpath(genpath(fullfile(Basefolder, 'DDTBOX-master')));
         
        %% Select Subject Datasets and Discrimination Groups (dcgs)
        
        %load(fullfile("Data", "Ready_for_DDTBOX", "EEG_sorted_cond", i_signal, i_cond, "eeg_sorted_cond.mat"));
        ...
        study_name = sprintf('MVPA_%s_%s_%s_Cortex', traits(i_trait), [split(Eye_conditions(i_cond), filesep)]');                            
        
        % Base directory path (where single subject EEG datasets and channel locations files are stored)
        %bdir = fullfile('Data', 'Ready_for_DDTBOX', 'EEG_sorted_cond', 'Total', Eye_conditions(i_cond));
        bdir = fullfile(Datafolder, 'Ready_for_DDTBOX', 'EEG_sorted_cond', Eye_conditions(i_cond));
        
        if ~(isfolder(fullfile(Datafolder, 'MVPA_Output_Cortex', filesep)))                
            mkdir(fullfile(Datafolder, 'MVPA_Output_Cortex', filesep))
        end
        % output_dir = [fullfile('Data', 'MVPA_Output', filesep)];
        output_dir = fullfile(Datafolder, 'MVPA_Output_Cortex', filesep);
          
        % Filepaths of single subject datasets (relative to the base directory)
        %sbj_code = {fullfile("Data", "Ready_for_DDTBOX", "EEG_sorted_cond", 'Total', Eye_conditions(i_cond), "eeg_sorted_cond")}; % participants EEG
        %sbj_code_bfi = {[fullfile("Data", "Ready_for_DDTBOX", "Personality_scores", traits(i_trait), string_parts{1}, "eeg_sorted_cond_regress_sorted_cond")]}; % participants BFI
        sbj_code = {fullfile(Datafolder, "Ready_for_DDTBOX", "EEG_sorted_cond", Eye_conditions(i_cond), "eeg_sorted_cond")}; % participants EEG
        sbj_code_bfi = {fullfile(Datafolder, "Ready_for_DDTBOX", "Personality_scores", traits(i_trait), "Pre", "eeg_sorted_cond_regress_sorted_cond")}; % participants BFI
        ...
        % channel_names_file = 'chanlocs_sorted.mat'; % Name of the .mat file containing channel labels and channel locations
        % channellocs = [fullfile('Data', 'Ready_for_DDTBOX')]; % Path of the directory containing channel information file
        ...
        dcg_labels = {};
        dcg_labels{1} = sprintf('Level_of_%s', traits(i_trait));
        %dcg_labels{2} = 'B vs. D';
