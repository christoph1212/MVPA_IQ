...
traits = ["Agreeableness", "Conscientiousness", "Extraversion", "NegativeEmotionality", "OpenMindedness"];
...
parfor i_trait = 1:length(traits)

    for i_cond = 1:length(Eye_conditions)
        
        for i_signal = 1:length(signalsEEG)
            ...
            study_name = sprintf('MVPA_%s_%s_%s_%s', signalsEEG(i_signal), traits(i_trait), [split(Eye_conditions(i_cond), '\')]');                            
            
            % Base directory path (where single subject EEG datasets and channel locations files are stored)
            bdir = fullfile('Data', 'Ready_for_DDTBOX', 'EEG_sorted_cond', signalsEEG(i_signal), Eye_conditions(i_cond));
            
            % Output directory (where decoding results will be saved)
            if ~(isfolder(fullfile('Data', 'MVPA_Output')))                
                mkdir(fullfile('Data', 'MVPA_Output'))
            end
            output_dir = [fullfile('Data', 'MVPA_Output', filesep)];
              
            string_parts = split(Eye_conditions(i_cond), '\');
            % Filepaths of single subject datasets (relative to the base directory)
            sbj_code = {fullfile("Data", "Ready_for_DDTBOX", "EEG_sorted_cond", signalsEEG(i_signal), Eye_conditions(i_cond), "eeg_sorted_cond")}; % participants EEG
            ...
            n_rep_cross_val = 2; % How many repetitions of full cross-validation with re-ordered data?
            perm_test = 1; % Run decoding using permuted condition labels? 0 = no / 1 = yes
            permut_rep = 2; % How many repetitions of full cross-validation for permuted labels analysis?
            ...
            for dcg_set = 1:length(dcgs_for_analyses)
                ...
                for sbj = sbj_todo
                    ...
                    % decoding_erp(cfg); 
		    evalc('decoding_erp(cfg)'); % To suppress output
